...
Search defaults to 20 rows; Projects currently caps a page at 100. `q` is NFKC-normalized and split on whitespace. With the default `mode: "any"`, a row matches when ANY token matches path, title, summary text, metadata (tags), or the indexed body text (first 20,000 chars of note bodies, mindmap node texts, WBS item names) as a case-insensitive substring. Results are ordered by `matchedTokens` (most tokens matched first), then recency. `mode: "all"` requires every token to match (use it to narrow, not as the first attempt). The response echoes `appliedQuery { tokens, mode, fields }`; when a search returns zero hits, check it before concluding the knowledge is missing, and retry with fewer or more literal tokens. There is no kana/romaji or synonym normalization. The indexed body text is search-only and never returned — results carry derived summaries, not authoritative bodies. The rebuild covers Artifact folders, notes, files, and Mindmap documents. Artifact repair is required; Mindmap repair is reported under the `mindmaps` result and may return `{ status: "error", service: "mindmaps", ... }` without discarding the Artifact rebuild result. Rebuild only to repair observed drift.
