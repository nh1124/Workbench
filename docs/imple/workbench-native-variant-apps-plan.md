...
### ウィンドウを開くコマンドは async にすること（2026-08-03 確定）

**Tauri の同期コマンドはメインスレッドで走る。** ウィンドウ生成はイベントループの応答を待って
ブロックするため、同期コマンドから呼ぶと**自分が待っているループを自分で止めて `build()` が返らない**。
症状は「空白のウィンドウが出て閉じられない」。

アプリログで確定した比較:

| 経路 | 結果 |
|---|---|
| グローバルショートカット（プラグインのスレッド） | `builder returned` まで **140ms** で到達、`built` まで完走 |
| Tauri コマンド（メインスレッド） | `building` の後が**一切出ない** |

**リポジトリ内に先例があった。** `choose_sync_folder` / `choose_downloads_folder` /
`save_file_with_dialog` は既に `async`。いずれも `blocking_pick_folder` を呼ぶ =
イベントループを待つ処理で、同じ理由で async 化されていた。ウィンドウ生成もダイアログと同種。

対処: ウィンドウを開くコマンドはすべて `pub async fn` にする（async コマンドは async runtime で走る）。

**遠回りの記録**（同じ道を辿らないために）:

1. 描画ゲートを疑って `main.tsx` を修正 → 実在の欠陥だったが原因ではない
2. 「メインスレッド以外からのビルドが悪い」と推測し `run_on_main_thread` へ退避 →
   **逆方向。すでにメインスレッドにいたため悪化させた**
3. URL の不統一を疑って修正 → 実在の改善だが原因ではない
4. サブクラスを疑って抑止 → 原因ではない
5. **アプリログを導入** → 「`build()` が返らない」ことが判明
6. 動く経路（ショートカット）にも同じログを入れて比較 → 確定

**教訓: 2 回目に直らなかった時点で計装すべきだった。** また「main では動く」という前提を
未検証のまま 3 ラウンド使った。比較対象は揃えてから語ること。

### P2-7 で踏んだ罠

- **ウィンドウプロシージャから Tauri API を同期呼び出ししてはいけない**。`WM_NCMOUSEMOVE` は頻繁に飛び、
  `webview_windows()` はウィンドウ生成中に握られているロックを取りにいくため、メインスレッドで再入して
  デッドロックし**ウィンドウが一切表示されなくなる**。
  `run_on_main_thread` でメインループへ委譲すること。
  **2026-08-03 補足**: 2026-08 に「専用アプリから Quick Note を開くと固まる」が再発した際、
  この罠を疑って**ウィンドウ生成中はサブクラスを沈黙させる**カウンタを入れたが、**これは誤診だった**。
  真因は同期コマンドからのウィンドウ生成（下記「ウィンドウを開くコマンドは async に」）。
  カウンタは将来のインシデント要因になりうるため**除去済み**。この罠自体は P2-7 当時のもので有効。
...
**(b)「プロセスが生きていれば使用中」で暫定確定**した。

**2026-08-04 に解消済み。** 反転が完了し `residentMode` を撤去した結果、アプリは最後のウィンドウと
一緒に終了するようになったため、「プロセスが生きている」＝「ウィンドウがある」となり、
**(b) の実装がそのまま (a) の意味になった**。以降の本節の記述は当時の経緯として読むこと。
