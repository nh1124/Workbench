...
- **ウィンドウプロシージャから Tauri API を同期呼び出ししてはいけない**。`WM_NCMOUSEMOVE` は頻繁に飛び、
  ...
  **2026-08-03 訂正: 委譲では防げない。** メインスレッド上ではイベントループが `build()` の中にいるため
  委譲先が動かず、実質同期実行になる。専用アプリから Quick Note を開くと `build()` が返らない、という形で
  再発した（アプリログの `building ...` に `built` が続かないことで確定）。
  **ウィンドウ生成中はサブクラスから Tauri を触らせない**（`titlebar::begin_window_build` /
  `end_window_build` のカウンタで抑止）。ホバー表示や最大化トグルはウィンドウが出来てからで足りる。
