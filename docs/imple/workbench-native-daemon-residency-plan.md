...
`exitWhenIdle`（UI 名「Quit Workbench When Idle」）= on のとき:
最後のウィンドウが閉じる → リース解放 → 猶予 60 秒 → daemon 終了 → **resident も終了しトレイから消える**。
グローバルショートカットもそこで効かなくなる。スタートメニューからアプリを開けば両方戻る。
...
### 仕様変更: `exitWhenIdle` は resident ごと終了させる（2026-08-06 ユーザー決定）

**UI 名も変えた: 「Stop Daemon When Idle」→「Quit Workbench When Idle」。**

もともとこの設定は sync daemon だけを止め、トレイアイコンは残る仕様だった。実機確認で
ユーザーが「終了しない」と報告し、調べると daemon は正しく終了していた——見ていたのは
トレイアイコン（resident）だった。

これは名前の問題ではなく**観測の問題**。ウィンドウが 1 つも無いとき、ユーザーから見える
Workbench はトレイアイコンだけで、daemon は不可視。**アイコンが残る限り「止まっていない」
と読むのが自然。** ユーザーの判断は「アイドル時はトレイからも消えてほしい」。

実装は `native/resident/src/main.rs` の `watch_for_idle`。10 秒ごとに次を見て、
すべて成り立てば resident を終了する:

1. `exitWhenIdle` が on
2. daemon が動いていない
3. Workbench のアプリウィンドウが 0

**猶予期間を自前で持っていない**のがポイント。daemon 自身が「最後のリース解放から 60 秒」
待ってから終了するので、**daemon が消えていること自体が「60 秒以上使われていない」の証明**に
なっている。二重に待つ必要がない。

「daemon を一度でも見た」フラグを条件に入れてある。これが無いと、daemon の起動に失敗した
ケースと「誰も必要としないので終了した」ケースが区別できず、ログイン直後に resident が
何もせず消える。

**代償（承知のうえ）**: resident が居ないのでグローバルショートカットが効かなくなる。
スタートメニューからアプリを開けば resident も daemon も戻る
（`resident::ensure_running`）。UI の説明文にもそう書いてある。
