...
##### 発見 → 修正済み: MindmapsPage の `isDirty` は書き込み専用だった（`0a88bbb`）
...
プロダクト判断としてユーザーに選択を仰ぎ、**(a) WBS と同様に `/ Unsaved` を出す**を採用。
`isDirty` の状態遷移は既に正しく揃っていた（編集で `true`、load / create / save / delete /
切替で `false`）ため、**表示を繋いだだけで state 変更は不要**。テスト 2 件で pin
