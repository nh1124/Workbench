...
const internalApiKey = requireEnv("INTERNAL_API_KEY_INSIGHTS");
...
function parseClaims(token: string): { coreUserId: string; username: string } {
  const decoded = jwt.verify(token, jwtSecret, { algorithms: ["HS256"], issuer: jwtIssuer });
  ...
  if (!sub || !username || tokenUse !== "access") throw new Error("Invalid token claims");
  return { coreUserId: sub, username: username.trim().toLowerCase() };
}
...
export const requireUserAuth: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if (!account) { res.status(403).json({ message: "Service account not provisioned" }); return; }
    req.authUser = {
...
export const requireInternalApiKey: RequestHandler = (req, res, next) => {
  ...
  if (!provided || provided !== internalApiKey) { res.status(403).json({ message: "Forbidden" }); return; }
  next();
};
