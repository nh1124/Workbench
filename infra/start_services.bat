...
for /f "usebackq delims=" %%P in (`node "%SCRIPT_DIR%\scripts\workbench-env.mjs" ports`) do set "WORKBENCH_PORTS=%%P"
...
cd /d "%PROJECT_ROOT%"
docker compose up -d --build artifacts-db artifacts workbench-core-db notes-db tasks-db projects-db images-db mindmaps-db wbs-db insights-db analyser-db
if errorlevel 1 (
  echo [ERROR] Failed to start PostgreSQL containers. Please ensure Docker Desktop is running.
  exit /b 1
)
call npm run dev:services:no-artifacts
